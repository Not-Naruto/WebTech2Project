...
app.get("/:stationID/recordSales", async (req, res) => {
  ...
  if (sd.userID == station.ManagerID) {
    ...
    res.redirect(
      `/Manager/${sd.userID}?msg=Lacking permissions to access screen`
    );
  }
});
...
app.post("/:stationID/recordSales", async (req, res) => {
  ...
  if (
    stationData.Fuel[0].remaining < premiumFuel ||
    stationData.Fuel[1].remaining < superFuel
  ) {
  ...
  res.redirect(
    `/Manager/${req.body.managerID}?msg=Sales has been added/Updated`
  );
});
...
app.post("/admin/:adminID/AddStation", async (req, res) => {
  const stationData = {
    ...
    Fuel: [
      {
        type: "Premium",
        price: parseFloat(req.body.premiumFuelPrice),
        remaining: 0,
      },
      {
        type: "Super",
        price: parseFloat(req.body.superFuelPrice),
        remaining: 0,
      },
    ],
  };
